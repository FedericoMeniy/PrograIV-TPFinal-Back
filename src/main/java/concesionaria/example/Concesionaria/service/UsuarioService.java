...
public class UsuarioService implements UserDetailsService {
    ...
    public void toggleFavorito(String emailUsuario, Long idPublicacion) {
        ...
        if (usuario.getFavoritos().contains(publicacion)) {
            usuario.getFavoritos().remove(publicacion);
        } else {
            usuario.getFavoritos().add(publicacion);
        }
    ...
    public void enviarCorreoRecuperacion(String email) {
        Usuario usuario = usuarioRepository.findByemail(email)
                .orElseThrow(() -> new RuntimeException("Si el correo existe en nuestro sistema, recibirás un enlace de recuperación."));
